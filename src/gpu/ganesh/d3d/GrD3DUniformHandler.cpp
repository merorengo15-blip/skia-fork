...
GrD3DUniformHandler::GrD3DUniformHandler(GrGLSLProgramBuilder* program)
    : INHERITED(program)
    , fUniforms(kUniformsPerBlock)
    , fSamplers(kUniformsPerBlock) {}

const GrShaderVar& GrD3DUniformHandler::getUniformVariable(UniformHandle u) const {
    return fUniforms.item(u.toIndex()).fVariable;
}

const char* GrD3DUniformHandler::getUniformCStr(UniformHandle u) const {
...
GrGLSLUniformHandler::SamplerHandle GrD3DUniformHandler::addSampler(
    ...
    // For D3D12, we use HLSL register bindings instead of SPIRV descriptor sets
    SkString layoutQualifier = SkStringPrintf("direct3d, set = %d, sampler = %d, texture = %d",
                                              kSamplerTextureDescriptorSet,
                                              binding,
                                              binding + 1);

    D3DUniformInfo& uniformInfo = fSamplers.emplace_back();
...
uint32_t GrD3DUniformHandler::getRTFlipOffset() const {
